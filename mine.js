...
module.exports.config = {
	...
	try {
		...
		if ("inventory" == n[0]) {
			const n = await axios.get(`https://project-mine.hbaoluvhue.repl.co/inventory?uid=${i}`),
				s = n.data.weapons.name || "Không có",
				t = n.data.pickaxe.name || "Không có",
				r = n.data.weapons.duribility || "Không có",
				d = n.data.pickaxe.duribility || "Không có";
			return e.sendMessage(`Vũ Khí : ${s}\nĐộ bền :${r}\n\nCúp : ${t}\nĐộ bền : ${d}`, a.threadID)
		}
	} catch (e) {
		console.log(e)
	}
				...
				case "7": {
					...
					if ("chuadangky" == s.data.msg) return e.sendMessage("[ HỆ THỐNG MINE ] - Bạn chưa đăng ký hệ thống", a.threadID); {
						const r = s.data.damage,
							...
							c = s.data.msg;
						s.data.images;
						if ("die" == s.data.msg) return i < 2000 ? e.sendMessage("[ HỆ THỐNG DUNGEON ] - Số Dư Của Bạn Không Đủ Đề Hồi Sinh ! Vui lòng kiếm đủ 2000$ để hồi sinh", a.threadID) : (await t.decreaseMoney(a.senderID, 2000), e.sendMessage("[HỆ THỐNG DUNGEON] - Bạn đã chết ! Hệ Thống Sẽ Tự Động Hồi Sinh Với Cái Gía Là 2000$", a.threadID));
		...
		case "getpickaxe":
			if (a.body == a.body) {
				if (i < c) return e.sendMessage("[ HỆ THỐNG MINE ] - Bạn không đủ tiền để mua cúp", a.threadID); {
					...
					if ("false" == n.data.data) return e.sendMessage("[ HỆ THỐNG MINE ] - Bạn chưa đăng ký hệ thống", a.threadID);
					if ("max" == n.data.msg) return e.sendMessage("[ HỆ THỐNG MINE ] - Bạn chỉ có thể mua 1 cúp", a.threadID);
